            ...
            <div className="flex flex-wrap gap-3">
              {footerLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="rounded-full border border-white/10 px-4 py-2.5 text-xs uppercase tracking-[0.26em] text-white/56 transition hover:border-[#d6b98c]/28 hover:text-white"
                >
                  {link.label}
                </Link>
              ))}
